  ...
  Future<void> fetchStationsAndCreateMarkers() async {
    try {
      ...
      snapshot.docs.forEach((doc) {
        final double s_lat = double.parse(doc['lat'] as String);
        final double s_lng = double.parse(doc['lng'] as String);
        final String name = doc['name'] as String;

        LatLng _stationLocation = LatLng(s_lat, s_lng);

        final marker = Marker(
          markerId: MarkerId(name),
          position: LatLng(s_lat, s_lng),
          onTap: () async {
            // Generate polyline when marker is tapped
            List<LatLng> polylineCoordinates =
                await getPolylinePoints(LatLng(s_lat, s_lng));
            ...
            if (_currentP != null) {
              ...
              // If the distance is less than or equal to 2 meters, show the QR button
              if (_distance <= 1) {
                // 0.002 km is equal to 2 meters
                setState(() {
              ...
              showDialog(
                ...
                builder: (BuildContext context) {
                  return AlertDialog(
                    title: Text(name),
                    content: Text(
                        'Distance to location: ${_distance.toStringAsFixed(2)} km'),
                    actions: <Widget>[
                      TextButton(
                        child: Text('OK'),
                        onPressed: () {
